...
try {
    ...
    // Obtener información del bono si la cita está asociada a uno
    $tiene_bono = false;
    $bono_info = null;
    ...
    if ($cita['es_bono'] && $cita['bono_id']) {
        $query_bono = "SELECT b.*, 
                       ...
                       FROM bonos b
                       WHERE b.id = :bono_id";
        $stmt_bono = $db->prepare($query_bono);
        $stmt_bono->bindParam(':bono_id', $cita['bono_id'], PDO::PARAM_INT);
        $stmt_bono->execute();
        
        if ($stmt_bono->rowCount() > 0) {
            $tiene_bono = true;
            $bono_info = $stmt_bono->fetch(PDO::FETCH_ASSOC);
        }
    }
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6 text-md-end">
                            ...
                            <span class="badge bg-success ms-2 px-3 py-2">
                                <i class="fas fa-ticket-alt me-1"></i> Bono
                            </span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-success border-0 d-flex align-items-center mb-4">
                        <i class="fas fa-ticket-alt me-3 fa-lg"></i>
                        <div>
                            <strong>Esta cita forma parte de un bono.</strong><br>
                            Sesiones disponibles: <strong><?= $bono_info['num_sesiones_disponibles'] ?> de <?= $bono_info['num_sesiones_total'] ?></strong>
                            <?php if ($bono_info['fecha_caducidad']): ?>
                            <br>Fecha de caducidad: <strong><?= date('d/m/Y', strtotime($bono_info['fecha_caducidad'])) ?></strong>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endif; ?>
