...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        // Actualizar el consentimiento con el nuevo token
        try {
            $db = getDB();
            $stmt = $db->prepare("
                UPDATE consentimientos_enviados 
                SET token = ?, email = ?, fecha_envio = NOW(), estado = 'pendiente' 
                WHERE id = ?
            ");
            $stmt->execute([$token, $email, $consentimiento_id]);
            ...
            setAlert('danger', 'Error al actualizar el consentimiento: ' . $e->getMessage());
        }
    }
}
